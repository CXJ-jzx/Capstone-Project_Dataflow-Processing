...
adjust_parallelism() {
    ...
    log "开始调整并行度: 当前 -> ${NEW_PARALLELISM}"
    
    # 检查是否有足够的 slot
    local AVAILABLE_SLOTS=$(get_available_slots)
    local TOTAL_TM=$(get_total_tm_count)
    
    # 如果 slot 不够，先扩容 TM
    if [[ ${NEW_PARALLELISM} -gt $((TOTAL_TM * 2)) ]]; then  # 假设每个 TM 2 个 slot
        log "Slot 不足，需要先扩容 TM"
        add_tm
        sleep 10  # 等待 TM 启动
    fi
    ...
    # 创建 Savepoint
    log "创建 Savepoint..."
    local SAVEPOINT_RESULT
    SAVEPOINT_RESULT=$(curl -s -X POST "${FLINK_REST_URL}/jobs/${FLINK_JOB_ID}/savepoints" \
        -H "Content-Type: application/json" \
        -d '{"cancel-job": false}')
    
    local TRIGGER_ID=$(echo "${SAVEPOINT_RESULT}" | jq -r '.["request-id"] // empty')
    
    if [[ -z "${TRIGGER_ID}" ]]; then
        ${ALERT_SCRIPT} "Savepoint 触发失败: ${SAVEPOINT_RESULT}"
        return 1
    fi
    
    # 等待 Savepoint 完成
    log "等待 Savepoint 完成 (trigger-id: ${TRIGGER_ID})..."
    local SAVEPOINT_PATH=""
    for i in {1..60}; do
        sleep 2
        local STATUS_RESPONSE=$(curl -s "${FLINK_REST_URL}/jobs/${FLINK_JOB_ID}/savepoints/${TRIGGER_ID}")
        local STATUS=$(echo "${STATUS_RESPONSE}" | jq -r '.status.id // "UNKNOWN"')
        
        if [[ "${STATUS}" == "COMPLETED" ]]; then
            SAVEPOINT_PATH=$(echo "${STATUS_RESPONSE}" | jq -r '.operation.location // empty')
            break
        elif [[ "${STATUS}" == "FAILED" ]]; then
            local FAILURE=$(echo "${STATUS_RESPONSE}" | jq -r '.operation["failure-cause"].class // "Unknown"')
            ${ALERT_SCRIPT} "Savepoint 失败: ${FAILURE}"
            return 1
        fi
    done
    
    if [[ -z "${SAVEPOINT_PATH}" ]]; then
        ${ALERT_SCRIPT} "Savepoint 超时"
        return 1
    fi
    ...
    # 从 Savepoint 重启作业（使用新并行度）
    log "从 Savepoint 重启作业，新并行度: ${NEW_PARALLELISM}"
    
    local RUN_RESULT
    RUN_RESULT=$(${FLINK_BIN_PATH}/flink run -d \
    ...
    if [[ $? -eq 0 ]]; then
        # 提取新的 Job ID
        local NEW_JOB_ID=$(echo "${RUN_RESULT}" | grep -oE '[a-f0-9]{32}' | tail -1)
        if [[ -n "${NEW_JOB_ID}" ]]; then
            log "作业重启成功，新 Job ID: ${NEW_JOB_ID}"
            # 更新配置文件中的 Job ID
        ...
        ${ALERT_SCRIPT} "作业重启失败: ${RUN_RESULT}，Savepoint 路径: ${SAVEPOINT_PATH}"
        return 1
    fi
}
