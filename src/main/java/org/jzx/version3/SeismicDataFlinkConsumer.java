...
public class SeismicDataFlinkConsumer {
    ...
    public static class OptimizedRocketMQSource extends RichParallelSourceFunction<SeismicRecord> implements CheckpointedFunction {
        ...
        public void snapshotState(FunctionSnapshotContext context) throws Exception {
            // Note: We persist offsets into Flink operator state to allow restart from the last checkpoint.
            // However, because RocketMQ does not participate in Flink's checkpoint 2PC, this approach provides
            // at-least-once delivery semantics. To achieve exactly-once with an external broker, additional
            // transactional protocols or external offset commit logic are required.
            if (offsetState != null) {
                offsetState.clear();
                if (offsetMap != null) {
                ...
                System.out.println("🔖 snapshot offsets saved: " + (offsetMap == null ? "{}" : offsetMap));
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        // ========== 【新增】缓存处理算子 ==========
        DataStream<SeismicRecord> cachedStream = validSeismicStream
                .keyBy(SeismicRecord::getSensorId)
                .process(new SeismicCacheProcessFunction())
                .name("Adaptive-Cache-Layer");

        // 4. 窗口聚合（使用 cachedStream 替代 validSeismicStream）
